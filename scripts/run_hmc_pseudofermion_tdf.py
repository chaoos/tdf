#!/usr/bin/env python3
"""Run TDF pseudofermion HMC for the 2-flavour Schwinger model."""
...
from tdf.hmc_pseudofermion import run_hmc_pseudofermion_tdf
...
def main():
    parser = argparse.ArgumentParser(
        description="TDF pseudofermion HMC for the 2-flavour Schwinger model"
    )
    ...
    logger.info("Running TDF pseudofermion HMC on %dx%d lattice", args.L, args.Lt)
    ...
    jnp.savez(
        "hmc_pseudofermion_tdf_summary.npz",
        plaquette=plaquette,
        topological_charge=q,
        accept=accept,
        delta_H=delta_H,
    )
    logger.info("Summary saved to hmc_pseudofermion_tdf_summary.npz")
